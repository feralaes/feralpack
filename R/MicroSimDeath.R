...
#' @param a.Transitions Transition probability array  (n.s x n.s x n.t x n.i)
...
MicroSimDeath <- function(a.Transitions, p.0, n.i, n.t, seed = NULL, cum = FALSE) {

  # Set the seed for the random number generator
  set.seed(seed)
  # Define the number of health states
  n.s <- dim(a.Transitions)[1]
  state_names <- dimnames(a.Transitions)[1]

  # Define the set of states
  states <- seq(1, n.s)
  # Create a matrix for storing individuals' sequence
  sequence <- matrix(nrow = n.i, ncol = n.t)
  rownames(sequence) <- ind_names     # Name the rows
  colnames(sequence) <- cycle_names   # Name the columns

  # Create a matrix to keep track of time spend in S1 or S2
  cum_sick <- matrix(0, nrow = n.i, ncol = n.t)
  rownames(cum_sick) <- ind_names     # Name the rows
  colnames(cum_sick) <- cycle_names   # Name the columns
  ...
  for (i in 1:n.i) {
    ...
    for (t in 2:n.t){
      ...
      probabilities <- a.Transitions[prev_state, , t, i]
      ...
      if (cum_sick[i, t] > 0 & t < n.t){
        a.Transitions [1, 4, , ] <- t(m.Die) * (1 + cum_sick[i, t] / 5) # Absolute increase of 20% (1/5) with every additional year of being sick
        a.Transitions [2, 4, t + 1, i] <- a.Transitions [1, 4, t + 1, i] * rr.S1  # Correct age specific mortality for beging sick
        a.Transitions [3, 4, t + 1, i] <- a.Transitions [1, 4, t + 1, i] * rr.S2  # correct age specific mortality for being sick

        # Calculate probabilities depending on other probabilities
        a.Transitions [1, 1, t + 1, i] <- 1 - a.Transitions [1, 2, t + 1, i] - a.Transitions [1, 4, t + 1, i]
        a.Transitions [2, 2, t + 1, i] <- 1 - a.Transitions [2, 1, t + 1, i] - a.Transitions [2, 3, t + 1, i] - a.Transitions [2, 4, t + 1, i]
        a.Transitions [3, 3, t + 1, i] <- 1 - a.Transitions [3, 4, t + 1, i]
      }
    }
  }
